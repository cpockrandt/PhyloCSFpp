...
uint8_t get_amino_acid_id2(const uint8_t i1, const uint8_t i2, const uint8_t i3)
{
    if (i1 == 4 || i2 == 4 || i3 == 4)
        return 64; // Marginalize "code"
    return 16 * i1 + 4 * i2 + i3;
}
...
void from_amino_acid_id(const uint8_t codon_id, uint8_t & i1, uint8_t & i2, uint8_t & i3)
{
    ...
    assert(codon_id == get_amino_acid_id2(i1, i2, i3));
}

char get_amino_acid(const uint8_t amino_acid_id)
{
    assert(amino_acid_id <= 64);
