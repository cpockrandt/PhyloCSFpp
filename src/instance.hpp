...
//                this->pi = other.pi; // NOTE: this might be important!!!!
            ...
            virtual ~q_diag_t()
            {
                ...
                {
                    // TODO: multiple elements can have the same pointer which would lead to a double-free. investigate this memory leak!
                    if (pi != NULL)
                    {
                        gsl_vector_free(pi);
                        pi = NULL;
...
//                    assert(pi == NULL); // TODO: either always keep allocated memory for have pi or get rid of bool "have_pi" and check whether pi == NULL
    ...
    void instantiate_qs(bool cpy = true) noexcept
    {
        ...
        // NOTE: sorting is not necessary, just helpful for comparing results with ocaml implementation
...
//    instance.p14n.tree_p14n = tree_array; // redundant work
    ...
    compute_q_p14ns_and_q_scale_p14ns_fixed_mle(instance, substitution_matrix, instance.q_settings);
