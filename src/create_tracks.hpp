...
std::vector<scored_region> create_track(hmm_parameter & hmm_params, std::string & input_file){
    ...
    uint32_t prev_pos = 0, block_start_pos = 0;
    std::string prev_chrom;
    while (fgets(line, sizeof(line), score_file)) {
        const char * delim = " \t";
        char *token = strtok(line, delim); //0
        std::string region(token);
        token = strtok(NULL, delim); //1
        std::string strand(token);
        token = strtok(NULL, delim); //2
        std::string data(token);
        data.pop_back();
        if(data == "No_Alignment"){
            continue;
        }
        token = strtok(NULL, delim); //3
        double score=atof(token);
        token = strtok(NULL, delim); //
        double rel_branch_length=atof(token);
        ...
        if(rel_branch_length > min_rel_branch_length){
            if(scores.size() == 0){
                block_start_pos = pos;
            }
